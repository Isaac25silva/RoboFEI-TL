...
void le_arquivo_casos()
{
	...
	//while(n_casos<100)
		...
		//cout <<"linha "<<n_casos<<" Valor 1 "<<CASOS_tetha1[n_casos]<<" Valor2 "<<CASOS_tetha2[n_casos]<<" Acao"<<CASOS_acao[n_casos]<<" Recomp "<<CASOS_reward_acao[n_casos]<<endl;
		
		if(CASOS_acao[n_casos]!=0)
		{
		  n_casos++;
		}
  ...
  for (int i = 0; i < CASOS_USADOS; ++i)
	{
		...
		//cout<<" similaridade "<< valorSimilaridade<<endl;
        if(valorSimilaridade < fatorsimilaridade) 
        {
        Anguloescolhido=CASOS_tetha2[i];
